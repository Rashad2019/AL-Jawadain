...
class SaleCommissionMakeInvoice(models.TransientModel):
    ...
    def _default_product_id(self):
        return self.env["product.product"].search([("default_code", "=", "Commission_001")])[:1]
    ...
    settlement_ids = fields.Many2many(
        ...
        domain="[('state', '=', 'settled'),('agent_type', 'in', ('salesperson','agent')),"
    ...
    def create_settlements_and_invoices(self):
        if self.settlement_ids:
            ...
            settlements = self.env["sale.commission.settlement"].search(
                [
                    ("state", "=", "settled"),
                    ('agent_type', 'in', ('salesperson','agent')),
